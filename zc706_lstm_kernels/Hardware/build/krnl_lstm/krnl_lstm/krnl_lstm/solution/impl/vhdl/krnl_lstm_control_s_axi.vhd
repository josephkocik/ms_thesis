...
entity krnl_lstm_control_s_axi is
generic (
...
end entity krnl_lstm_control_s_axi;
...
architecture behave of krnl_lstm_control_s_axi is
    ...
    constant ADDR_Y_H_DATA_0 : INTEGER := 16#18#;
    constant ADDR_Y_H_CTRL   : INTEGER := 16#1c#;
    ...
    signal int_y_h             : UNSIGNED(31 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_Y_H_DATA_0 =>
                        rdata_data <= RESIZE(int_y_h(31 downto 0), 32);
                    when others =>
                        NULL;
                    end case;
                end if;
...
-- ----------------------- Register logic ----------------
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_Y_H_DATA_0) then
                    int_y_h(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_y_h(31 downto 0));
                end if;
            end if;
        end if;
    end process;
